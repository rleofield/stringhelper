...
namespace {
   ...
   std::string  msgFail( string const& m, double testvalue, double correctvalue ) {
      return msgFail( m, strhlp::toString( testvalue ), strhlp::toString( correctvalue ) );
   }
...
void testPass( std::string f,  bool b, std::string const& msg ) {
   //f = strhlp::replace_first(f,"TestStrings::","");
   if( f.size() > 30 ) {
      f = f.substr( 0, 30 );
   }
...
void TestStrings::test_string_to_type_double() {
   ...
   double ist = strhlp::string2type<double>( "1234.7" );
   double soll  = 1234.7;
...
void TestStrings::test_string_to_type_int() {
   ...
   int ist  = strhlp::string2type<int>( "4711" );
   int soll = 4711;
...
void TestStrings::test_string_to_type_size_t() {
   ...
   size_t ist = strhlp::string2type<size_t>( temp );
   size_t soll = -1;
...
void TestStrings::test_double_to_string_default_format() {
   ...
   string ist = strhlp::toString( 1234.7 ); // 1234.700, 3 dezimalstellen, default
...
void TestStrings::test_double_to_string_format_supplied() {
   ...
   string ist = strhlp::toString( 1234.7,  10, 7 ); // 1234.7000000, 7 nachkommastellen, user defined
...
void TestStrings::test_int_to_hex_0x() {
   ...
   // 32 bit
   string ist = strhlp::to_hex_0x( a );
   // 64 bit
   //string ist = strhlp::to_hex_0x( 18446744073709551615ul ) ; // mit 0x "0xFFFFFFFFFFFFFFFF
   ...
   VERIFY( ist == soll, msgFail( "strhlp::to_hex_0x( " + temp + " )", ist, soll ) );
}
...
void TestStrings::test_int_to_hex_0x_32bit() {
   ...
   string ist = strhlp::to_hex_0x( 4294967295 ) ; // mit 0x "0xFFFFFFFF
   string soll = "0xFFFFFFFF";
   VERIFY( ist == soll, msgFail( "strhlp::to_hex_0x( 4294967295 )", ist, soll ) );
}
...
void TestStrings::test_int_to_hex_0x_example2() {
   ...
   string ist = strhlp::to_hex_0x( 1234567 ); // 0x0012D687
...
void TestStrings::test_int_to_hex_0x_width16() {
   ...
   string ist = strhlp::to_hex_0x( 1234567, 16 );
...
void TestStrings::test_int_to_hex() {
   ...
   string ist = strhlp::to_hex( 256 * 256 + 1 ); // ohne 0x "00010001"
...
void TestStrings::test_int_to_binaer_string() {
   ...
   string ist = strhlp::to_bin( 256 * 256 - 1 );
...
void TestStrings::test_int_to_binaer_string_width16() {
   ...
   string ist = strhlp::to_bin( 256 * 256 - 1, 16 );
...
void TestStrings::test_to_lower_in_place() {
   ...
   strhlp::to_lower_in_place( upper ); // "kapitaelchen"
   string ist = upper;
...
void TestStrings::test_to_lower_copy_string() {
   ...
   string ist = strhlp::to_lower( "TEST mit KOPIE DES STRINGS" );
...
void TestStrings::test_to_upper_in_place() {
   ...
   strhlp::to_upper_in_place( lower );
   string ist = lower;
...
void TestStrings::test_to_upper_copy_string() {
   ...
   string ist = strhlp::to_upper( "abcdfghi++??u" );
...
void TestStrings::test_clip_before_pattern() {
   ...
   string ist = strhlp::clip( " 1234pattern1234, ", "pattern" );
   string soll = " 1234" ;
...
void TestStrings::test_clip_after_pattern() {
   ...
   string ist = strhlp::clip_after( "hgxxxxxypattxy xy " , "patt" ) ;
...
void TestStrings::test_clip_at_pos() {
   ...
   string ist = strhlp::clip_at_pos( "12 34pattern1234", 6 );
   string soll =  "12 34p" ;
...
void TestStrings::test_clip_at_char() {
   ...
   string ist = strhlp::clip_at_char( "123 4pattern12W34, ", 'W' ) ;
...
void TestStrings::test_truncate_at_pos() {
   ...
   string ist = strhlp::truncate( "hier ist das 1234 ", 7 );
   string soll = "hier is" ;
...
void TestStrings::test_trim() {
   ...
   string ist = strhlp::trim( trimtest ) ;
...
void TestStrings::test_trim_right() {
   ...
   string ist = strhlp::trim_right( trimtest );
   string soll = "    spaces before    and     spaces after" ;
...
void TestStrings::test_trim_left() {
   ...
   string ist = strhlp::trim_left( trimtest );
   string soll = "spaces before    and     spaces after    " ;
...
void TestStrings::test_trim_with_empty_string() {
   ...
   ist = strhlp::trim_right( trimtest ) ;
   soll = "" ;
   VERIFY( ist == soll, msgFail( "trim_right(), mit Leerstring", ist, soll ) );

   ist = strhlp::trim_left( trimtest ) ;
   soll = "" ;
...
void TestStrings::test_index_at_char() {
   ...
   size_t ist = strhlp::index( "einW an Position 3", 'W' ) ;
   size_t soll = 3 ;
...
void TestStrings::test_index_at_pattern() {
   ...
   size_t ist = strhlp::index( "ein W an Position 4", "W an " ) ;
   size_t soll = 4 ;
...
void TestStrings::test_index_right_at_char() {
   ...
   size_t ist = strhlp::index_right( "einW an Position 20 W ", 'W' ) ;
   size_t soll = 20;
...
void TestStrings::test_index_at_pattern_from_pos() {
   ...
   size_t ist = strhlp::index( "hgxx123456hg12hgEE fy", "hg", 3 ) ;
...
void TestStrings::test_index_right_at_pattern() {
   ...
   size_t ist = strhlp::index_right( "hgxxxyzWpatternWfxyzgfyeg fy", "xyz" ) ;
   size_t soll = 17;
...
void TestStrings::test_index_not_found() {
   ...
   ist = strhlp::index_right( "einW an Position W ", 'w' ) ;
   soll = string::npos ;
...
void TestStrings::test_replace_first() {
   ...
   string ist = strhlp::replace_first( "hgxxxxhghfhxxeg fy", "xx", "1234567890" ) ;
...
void TestStrings::test_replace_all() {
   ...
   string ist = strhlp::replace_all( "hgxxxxhgxxxhfgxydxxxxxxxxxhWxypattxy xy ", "xx", "yy1234" ) ;
...
void TestStrings::test_fillup() {
   ...
   string ist = strhlp::fillup( "abc", 'k', 6 ) ;
   string soll = "abckkk";
...
void TestStrings::test_erase() {
   ...
   string ist = strhlp::erase( "hgxxxxFFhgh fy", 6, 2 ); // FF wird entfernt
...
void TestStrings::test_erase_in_place() {
   ...
   strhlp::erase_in_place( testErase, 2, 1 ); // 3 wird entfernt, returns void
...
void TestStrings::test_remove_last() {
   ...
   string ist = strhlp::remove_last( "hgxxxxhghfgx fy" ) ;
...
void TestStrings::test_remove_last_in_place() {
   ...
   strhlp::remove_last_in_place( testForRemove ) ;   // returns void
...
void TestStrings::test_remove_first() {
   ...
   string ist = strhlp::remove_first( "hgxxxxh fy" ) ;
...
void TestStrings::test_remove_first_pattern() {
   ...
   string ist = strhlp::remove_first( "hgxxhfgx-ydhWxypxy x-y  fy", "x-y" ) ;
...
void TestStrings::test_insert_at_char() {
   ...
   string ist = strhlp::insert_at( "hgxxxxheg fy", 'A', 3 ) ;
...
void TestStrings::test_insert_at_pattern() {
   ...
   string ist = strhlp::insert_at( "hgxxxxhgxxxh fy", "ABC", 3 ) ;
...
void TestStrings::test_split_to_6_tokens() {
   ...
   // 6 tokens
   std::vector<std::string> v = strhlp::split( "Fuchs, du ha,st die Gans gestohlen.", " " );
