...
void test() {
   ...
   // Umwandlung eines Strings in einen Wert, Beispiel nach 'size_t'
   {
      ...
      size_t ist = strhlp::string2type<size_t>( "4294967295" );
      // 64 bit
   ...
   // Int-Wert in einen hexadezimalen String, mit '0x' Prefix, nur mit 64 bit
   {
      ...
      string ist = strhlp::to_hex_0x( a ) ; // mit 0x "0xFFFFFFFFFFFFFFFF
...
int main( void ) {
   ...
   // hier ist der simple test(), andere Tests sind im Projekt 'tests'
   std::string s = ral::strings::to_hex_0x( 5 );
   cout << "abc" << endl;
   test();
   return 0;
}
